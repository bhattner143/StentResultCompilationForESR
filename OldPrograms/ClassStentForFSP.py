...
#        pdb.set_trace()
        self.files=[files[index] for root, dirs, files in sorted(os.walk(self.path,topdown=True)) for index in range(len(files)) if name in files[index]] 
...
# =============================================================================
    def CreateDataframe(self,*Window,ColumnName=['Value1','Value2','Value3'],dataArray=np.array([])):
        ...
        final=Window[0][1]
        if len(dataArray)==0:  
            self.dataframe = pd.DataFrame({ColumnName[0]:self.df1[initial:final,1],ColumnName[1]:self.df2[initial:final,1],ColumnName[2]:self.df3[initial:final,1]})
